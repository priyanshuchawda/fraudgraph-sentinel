...
from fraudgraph_sentinel.aura_config import load_neo4j_config, read_env_file, required_env_status
...
def test_load_neo4j_config_rejects_missing_required_values(tmp_path):
    env_file = tmp_path / ".env"
    env_file.write_text("NEO4J_URI=neo4j+s://example.databases.neo4j.io\n", encoding="utf-8")
...
def test_load_neo4j_config_rejects_non_neo4j_uri(tmp_path):
    ...
    assert "NEO4J_URI must start with neo4j+s://, neo4j+ssc://, bolt+s://, bolt+ssc://, neo4j://, or bolt://" in str(error.value)
